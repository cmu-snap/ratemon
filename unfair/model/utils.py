...
class BalancedSampler:
    ...
    def __init__(self, dataset, batch_size, drop_last, drop_popular):
        ...
        if batch_size is None:
            ...
            num_clss = len(clss)
            logging.info(
                "Dropped %d samples to enable BalancedSampler with no batch size.",
                to_drop,
            )
        ...
        if drop_popular:
            ...
            for cls, cls_idxs in clss_idxs.items():
                ...
                if num_examples > target_examples:
                    ...
                    logging.info(
                        "\tRemoved %d examples from class %s.",
            ...
            for cls, cls_idxs in clss_idxs.items():
                ...
                if num_examples < target_examples:
                    ...
                    logging.info("\tAdded %d examples to class %s.", new_examples, cls)
...
class Exp:
    ...
    def __init__(self, sim):
        ...
        self.cca_1_flws = (
            cca_1_flws
            if isinstance(cca_1_flws, int)
            else int(cca_1_flws[: -(len(self.cca_1_name))])
        )
        # Number of CCA 2 flows.
        self.cca_2_flws = (
            cca_2_flws
            if isinstance(cca_2_flws, int)
            else int(cca_2_flws[: -(len(self.cca_2_name))])
        )
        # Number of CCA (background) flows.
        self.cca_back_flws = (
            cca_back_flws
            if isinstance(cca_back_flws, int)
            else int(cca_back_flws[: -(len(self.cca_back_name))])
        )
...
def parse_packets(flp, flw_to_cca, local_ip, select_tail_percent=None):
    ...
    """
    logging.info("\tParsing PCAP: %s", flp)
    ...
    if num_pkts == 0:
        logging.info("No packets found in: %s", flp)
    else:
        logging.info(
            "\tDiscarded packets: %s (%.2f%%)",
            discarded_pkts,
            discarded_pkts / num_pkts * 100,
        )
    return flw_to_pkts
...
def parse_queue_log(flp):
    ...
    """
    logging.info("\tParsing queue log: %s", flp)
...
def load_exp(flp, msg=None):
    ...
    logging.info("%sParsing: %s", "" if msg is None else f"{msg} - ", flp)
    exp = Exp(flp)
    try:
        with np.load(flp, allow_pickle=True) as fil:
            ...
            if num_files == exp.tot_flws:
                ...
                logging.info(
                    "\tThe number of subfiles (%s) does not match the number of flows (%s): %s",
                    num_files,
                    exp.tot_flws,
                    flp,
                )
        ...
        logging.info("Bad simulation file: %s", flp)
        dat = None
    return exp, dat
...
def save_parsed_data(flp, trn, val, tst):
    ...
    """
    logging.info("Saving data: %s", flp)
    np.savez_compressed(
        flp,
        train_in=trn[0],
        train_out=trn[1],
...
def load_parsed_data(flp):
    """Load parsed data."""
    logging.info("Loading data: %s", flp)
...
def save_tmp_file(flp, dat_in, dat_out, dat_extra, scl_grps):
    ...
    logging.info("Saving temporary data: %s", flp)
    np.savez_compressed(
...
def load_tmp_file(flp):
    ...
    logging.info("Loading temporary data: %s", flp)
    with np.load(flp) as fil:
        dat_in = fil["dat_in"]
        dat_out = fil["dat_out"]
...
def get_npz_headers(flp):
    ...
    try:
        ...
        logging.info("Warning: Not a valid zip file: %s", flp)
        return []
...
def save_scl_prms(out_dir, scl_prms):
    ...
    logging.info("Saving scaling parameters: %s", scl_prms_flp)
